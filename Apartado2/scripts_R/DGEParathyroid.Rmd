...
## LECTURA DE DATOS Y CONVERSIÓN DE VARIABLES A FACTORES
...
design(dds2)
```
# RESULTADOS CONTRASTES DISEÑO 1.
```{r}
...
if (nrow(significant_genes) == 0) {
  ...
  ## Heatmap 1 de los genes TOP DGE ordenados por abs(LFC)
  ...
  # Centro los datos para hacer el heatmap.
 ...
 ## Heatmap 2 de los genes TOP DGE ordenados por abs(LFC) teniendo en cuenta el paciente
...
if (nrow(significant_genes_c1) == 0) {
  ...
  ## Heatmap de los genes TOP DGE ordenados por abs(LFC)
...
## PREFILTRADO Y DGE PARA EL DISEÑO 2.
...
```
# RESULTADOS CONTRASTES DISEÑO 2
...
if (nrow(significant_genes_c1) == 0) {
  ...
  print(paste("El número de genes diferencialmente expresados con padj<0.05 para el contraste ",name2, "con el diseño 2 es", nrow(significant_genes_c1)))
  # Ordenar los resultados filtrados por el valor absoluto de log2FoldChange en orden descendente
  significant_genes_c1 <- significant_genes_c1[order(-abs(significant_genes_c1$log2FoldChange)), ]
  # Exportar la tabla con la columna adicional a un archivo CSV
...
if (nrow(significant_genes_c2) == 0) {
  ...
  pheatmap(res_top3heatmap, 
          ...
          cluster_cols = FALSE)
  # Anotación de genes en el heatmap
  gene_symbols_heatmap <- mapIds(org.Hs.eg.db, keys = rownames(res_top3heatmap), column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
  # Asignar los símbolos de los genes como nombres de fila en res_top2heatmap
  rownames(res_top3heatmap) <- gene_symbols_heatmap
  # Centrar datos.
